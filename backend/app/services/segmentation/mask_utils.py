"""Shared helpers for turning a binary mask into a polygon, used by every
point-prompt model implementation."""

from __future__ import annotations

import numpy as np
from skimage.measure import approximate_polygon, find_contours

DEFAULT_SIMPLIFY_TOLERANCE = 0.75  # lower tolerance keeps ~2x more points
MIN_POLYGON_POINTS = 3


def mask_to_polygon(
    mask: np.ndarray, tolerance: float = DEFAULT_SIMPLIFY_TOLERANCE
) -> list[list[float]] | None:
    """Vectorize the largest boundary of a binary mask into a simplified
    [x, y] polygon, or None if the mask has no usable contour."""
    contours = find_contours(mask.astype(np.float32), level=0.5)
    if not contours:
        return None

    largest = max(contours, key=len)
    simplified = approximate_polygon(largest, tolerance=tolerance)
    if len(simplified) < MIN_POLYGON_POINTS:
        return None

    # contours are (row, col); convert to (x, y) for the frontend
    return [[float(c), float(r)] for r, c in simplified]
